...
// DÜZELTME: 100'e bölme işlemi kaldırıldı. Veri direkt gösteriliyor.
  ...
  return (
    <div className="p-8 space-y-8 text-slate-100">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {/* VIP istatistikleri */}
        <div className="rounded-xl bg-[#020617] border border-white/5 p-6">
          <div className="text-lg font-semibold mb-4">VIP İstatistikleri</div>
          {vipStats ? (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          {lossSummary && lossSummary.count > 0 ? (
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {formatNumber(lossSummary.totalBonusFreespin)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="opacity-70">Toplam freebet:</span>
                <span className="font-medium">
                  {formatNumber(lossSummary.totalBonusFreebet)}
                </span>
              </div>
            </div>
          ) : (
        ...
        <div className="rounded-xl bg-[#020617] border border-white/5 p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="text-lg font-semibold">Kayıp Bonus Geçmişi</div>
            {lossMeta && (
                  ...
                  <tbody>
                    {lossHistory.map((row) => (
                      <tr key={row.id} className="align-middle">
                        ...
                        <td className="py-1 pr-4">
                          {formatNumber(row.addedFreespin)}
                        </td>
                        <td className="py-1 pr-4">
                          {formatNumber(row.addedFreebet)}
                        </td>
                        <td className="py-1 pr-4 text-right">
                          <span className="inline-flex items-center rounded-full bg-emerald-500/10 text-emerald-300 border border-emerald-500/40 px-2 py-[2px] text-[11px]">
                            {row.status}
